...
public:
    ...
    void process(const dsp::ProcessContextReplacing<float>& context) noexcept override
    {
        ...
        {
            channelStates[channel].compressor.setAttack(static_cast<double>(params->attack));
            channelStates[channel].compressor.setRelease(static_cast<double>(params->release));
            channelStates[channel].compressor.setThreshold(static_cast<double>(params->threshold));
            channelStates[channel].compressor.setRatio(1.0 / static_cast<double>(params->ratio));
